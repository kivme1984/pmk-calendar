...
(() => {
  if (globalThis.PMK_STATUS_PIPELINE_V80) return;
  globalThis.PMK_STATUS_PIPELINE_V80 = true;
  ...
  function updateVisibleCounters() {
    ...
    const list = state.currentView === 'delivery-waiting'
      ? all.filter(event => eventMeta(event).requestStatus === 'pending-delivery')
      : all.filter(event => eventDateKey(event) === state.selectedDayKey && !HIDDEN_STATUSES.has(eventMeta(event).requestStatus));
    const map = {
      todayCount: state.currentView === 'day' ? list.length : $('#todayCount')?.textContent,
    ...
    Object.entries(map).forEach(([id, value]) => {
      const node = $(`#${id}`);
      if (node != null && value != null) node.textContent = String(value);
    });
  }
  ...
  globalThis.PMK_STATUS_PIPELINE_V80_API = {
    applyStatus,
    prepareTransition,
    patchMatchingEvents,
