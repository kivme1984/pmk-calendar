const VERSION='82.45.0';
const CACHE=`pmk-calendar-v${VERSION}`;
const BUNDLE_JS='./__pmk-app-v82-45-0.js';
const BUNDLE_CSS='./__pmk-styles-v82-45-0.css';
...
async function textAsset(url){
  ...
  if(url.includes('status-left-column-v82-2.js')&&!text.includes('PMK_DAY_CARD_THIN_STATUS_V82_45'))throw new Error('Не получены тонкие кнопки статуса v82.45.0');
  ...
  if(url.includes('update-manager-v82-20.js')&&!text.includes('PMK_UPDATE_MANAGER_V82_45'))throw new Error('Не получен менеджер обновлений v82.45.0');
