...
self.addEventListener('activate',event=>event.waitUntil((async()=>{const keys=await caches.keys();await Promise.all(keys.filter(k=>key!==CACHE&&key!=='pmk-calendar-data-v68').map(k=>caches.delete(k)));await self.clients.claim();})()).catch(()=>self.clients.claim()));
