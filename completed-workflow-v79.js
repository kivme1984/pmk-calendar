...
(() => {
  ...
  function refreshOverrides() { overrides = loadOverrides(); }
  ...
  eventMeta = function eventMetaLegacyStatusAwareV79(event) {
    ...
    const override = overrides[eventKey(event, base)] || null;
    return {
      ...base,
      requestStatus: override?.status || descriptionStatus || base.requestStatus,
      contractNumber: override?.contractNumber || contractFromDescription(event?.description || '') || base.contractNumber || '',
      workStartedAt: override?.workStartedAt || base.workStartedAt || '',
      completedAt: base.completedAt || completionFromDescription(event?.description || '') || (descriptionStatus === 'completed' ? (event?.updated || '') : '') || (override?.status === 'completed' ? override.updatedAt : ''),
    };
  };

  function visibleInDay(event) { return !HIDDEN_DAY_STATUSES.has(eventMeta(event).requestStatus); }
  function completionTimestamp(event) {
    const data = eventMeta(event);
    const override = overrides[eventKey(event, data)] || null;
    return String(data.completedAt || override?.updatedAt || event.updated || event.created || `${eventDateKey(event)}T00:00:00`);
  }
  function completedEvents() {
    return getAllEvents().filter(event => eventMeta(event).requestStatus === 'completed').sort((a, b) => completionTimestamp(b).localeCompare(completionTimestamp(a)));
  }
  ...
  function installObserver() {
    ...
    const observer = new MutationObserver(() => { scrubTodayDom(); renderCompleted(); });
    observer.observe(container, { childList:true, subtree:true });
  }

  window.PMK_COMPLETED_WORKFLOW_V79_API = { render:renderCompleted, events:completedEvents, scrub:scrubTodayDom };
  const install = () => { createCompletedView(); installObserver(); scrubTodayDom(); renderCompleted(); };
  ...
  window.addEventListener('pmk-calendar-sync-done', () => requestAnimationFrame(() => { scrubTodayDom(); renderCompleted(); }));
  window.addEventListener('storage', event => { if (event.key === OVERRIDES_KEY) { refreshOverrides(); scrubTodayDom(); renderCompleted(); } });
})();