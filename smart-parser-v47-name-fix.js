...
(() => {
  ...
  const blocked = new Set([
    ...
    'район', 'ковер', 'ковёр', 'ковра', 'ковров', 'заказ', 'заявка', 'забор', 'доставка',
    'сегодня', 'завтра', 'первый', 'первого', 'второй', 'второго', 'два', 'три',
    'синтетика', 'шегги', 'размер', 'ворс', 'пятна',
  ]);
  ...
  function extractName(text = '') {
    const boundary = '(?:^|[\\s,.;:—–-])';
    const patterns = [
      new RegExp(`${boundary}зовут\\s+([а-яё-]+(?:\\s+[а-яё-]+){0,2})`, 'i'),
      new RegExp(`${boundary}имя\\s*(?:клиента|клиентки)?\\s*[:—–-]?\\s*([а-яё-]+(?:\\s+[а-яё-]+){0,2})`, 'i'),
      new RegExp(`${boundary}клиент(?:ка)?\\s*(?:это|будет)?\\s*[:—–-]?\\s*([а-яё-]+(?:\\s+[а-яё-]+){0,2})`, 'i'),
      new RegExp(`${boundary}(?:заявка|заказ)\\s+(?:на|от)\\s+([а-яё-]+(?:\\s+[а-яё-]+){0,2})`, 'i'),
      new RegExp(`${boundary}(?:позвонила|позвонил|обращается)\\s+([а-яё-]+(?:\\s+[а-яё-]+){0,2})`, 'i'),
    ];
  ...
  api.parseText = function parseTextWithNameFix(rawText) {
    ...
    const explicit = extractName(String(rawText || ''));
    return {
      ...parsed,
      customerName: explicit || parsed.customerName || '',
    };
  };
