...
(() => {
  ...
  document.documentElement.dataset.pmkCandidate = '82.12';
  ...
  function phoneForCard(card) {
    ...
    if (event) {
      try {
        ...
        const eventText = [event.description, event.summary, event.location, data.managerComment, data.timeNote]
          .filter(Boolean).join(' ');
        const match = eventText.match(/(?:\+?7|8)[\s()\-]*\d{3}[\s()\-]*\d{3}[\s\-]*\d{2}[\s\-]*\d{2}/);
  ...
  function lockFooter(card) {
    ...
    if (phone) {
      if (!primary || !primary.matches('a[href^="tel:"]')) {
        ...
        link.href = `tel:${phone}`;
        if (primary) primary.replaceWith(link);
        else row.prepend(link);
        primary = link;
      }
    ...
    const menu = $('.menu-button', row);
    if (menu) menu.classList.add('pmk-menu-locked-v82-12');
  }
  ...
  function installRenderHooks() {
    if (typeof renderToday === 'function' && !renderToday.__pmkLayoutLockV8212) {
      ...
      globalThis.renderToday = function renderTodayV8212(...args) {
    ...
    if (typeof renderSearch === 'function' && !renderSearch.__pmkLayoutLockV8212) {
      ...
      globalThis.renderSearch = function renderSearchV8212(...args) {
        const result = previous(...args);
        scheduleLock();
        return result;
      };
    ...
    if (typeof renderAll === 'function' && !renderAll.__pmkLayoutLockV8212) {
      ...
      globalThis.renderAll = function renderAllV8212(...args) {
        const result = previous(...args);
        scheduleLock();
        return result;
      };
