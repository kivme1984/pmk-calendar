...
(() => {
  ...
  function cleanNameCandidate(value = '') {
    ...
    for (const token of tokens) {
      ...
      if (/^(это|будет|у\s*нас|его|ее|её)$/i.test(normalized)) continue;
      accepted.push(normalized);
      if (accepted.length === 3) break;
    }

    if (!accepted.length) return '';
    if (accepted.some(token => token.length < 2)) return '';
    return titleCase(accepted.join(' '));
  }
  ...
  function extractBetterName(text = '', fallback = '') {
    ...
    const leading = firstSegment.match(/^([а-яё-]{2,30}(?:\s+[а-яё-]{2,30})?)\s+(?=телефон|номер|адрес|улица|два\s+ков|три\s+ков|ков[её]р)/i);
    const leadingCandidate = cleanNameCandidate(leading?.[1] || '');
    if (leadingCandidate) return leadingCandidate;

    return fallback || '';
  }

  function explicitRugCount(text = '') {
    const match = text.match(/(?:количество\s+ковр[а-я]*\s*[:—–-]?\s*)?(\d+|один|два|две|три|четыре|пять)\s*(?:штук[аи]?\s*)?ковр[а-я]*/i);
    return match ? Math.max(0, Math.min(10, numberValue(match[1]))) : 0;
  }

  function normalizeRugMarkers(text = '') {
    let value = String(text);
    value = value.replace(/\b((?:два|две|три|четыре|пять|[2-5])\s+ковр[а-я]*\s*[:—–,-]?\s*)(?:один|1)\b/gi, '$1 первый ковёр ');
    value = value.replace(/\bков[её]р\s*(?:номер\s*)?(?:один|1)\b/gi, 'первый ковёр');
    value = value.replace(/\bков[её]р\s*(?:номер\s*)?(?:два|2)\b/gi, 'второй ковёр');
    value = value.replace(/\bков[её]р\s*(?:номер\s*)?(?:три|3)\b/gi, 'третий ковёр');
    value = value.replace(/\bков[её]р\s*(?:номер\s*)?(?:четыре|4)\b/gi, 'четвёртый ковёр');
    value = value.replace(/\bков[её]р\s*(?:номер\s*)?(?:пять|5)\b/gi, 'пятый ковёр');
    return value;
  }
  ...
  function findRugMarkers(text = '') {
    ...
    const pattern = /\b(?:на|у|для)?\s*(?:ков[её]р[а-я]*\s*(?:номер\s*)?)?(перв(?:ый|ого|ом|ому)|втор(?:ой|ого|ом|ому)|трет(?:ий|ьего|ьем|ьему)|четв[её]рт(?:ый|ого|ом|ому)|пят(?:ый|ого|ом|ому)|[1-5](?:-?й)?)(?:\s+ков[её]р[а-я]*)?\b/gi;
    const markers = [];
    let match;
    ...
    while ((match = pattern.exec(normalized))) {
      ...
      const index = markerIndex(match[1]);
      if (index < 0) continue;
  ...
  function materialFromContext(text = '') {
    ...
    if (/шерстян|100\s*%\s*шерст|натуральн\w*\s+шерст/i.test(text)) return 'Шерсть';
  ...
  function pileFromContext(text = '') {
    if (/ш[еёа]гги|шагги|shaggy|травк|высок\w*\s+ворс|длинн\w*\s+ворс/i.test(text)) return 'Более 1 см';
    if (/безворс|без\s+ворс|циновк|килим|палас/i.test(text)) return 'Без ворса';
    if (/средн\w*\s+ворс|коротк\w*\s+ворс|низк\w*\s+ворс|обычн\w*\s+ворс/i.test(text)) return 'До 1 см';
    return '';
  }
  ...
  function servicesFromContext(text = '') {
    return unique([
      ...
      /расч[её]с|расчес|поднят(?:ие|ь|ься)\s*ворс|подъ[её]м\s*ворс|приподнят\w*\s+ворс/i.test(text) ? 'Подъём ворса' : '',
  ...
  function rugFromContext(context = '', fallback = {}) {
    ...
    const shaggy = /ш[еёа]гги|шагги|shaggy|травк/i.test(context);
    return {
      ...blankRug(),
      ...fallback,
      length: Number(dimensionRug.length || fallback.length || 0),
      width: Number(dimensionRug.width || fallback.width || 0),
      material: materialFromContext(context) || fallback.material || (shaggy ? 'Синтетика' : ''),
      pile: pileFromContext(context) || fallback.pile || (shaggy ? 'Более 1 см' : ''),
      issues: [],
      services: unique([...servicesFromContext(context), ...(fallback.services || [])]),
    };
  }
  ...
  function structuredRugs(text = '', baseRugs = []) {
    ...
    const hasStructuredContext = contexts.some(parts => parts.length);
    if (!hasStructuredContext && baseRugs.length) return baseRugs;

    return Array.from({ length: total }, (_, index) => {
      const context = contexts[index].join('. ');
      const fallback = baseRugs[index] || {};
      return context ? rugFromContext(context, fallback) : { ...blankRug(), ...fallback, issues: [], services: unique(fallback.services || []) };
    });
  }

  api.parseText = function parseTextV47(rawText) {
    const base = previousParseText(rawText);
    const text = String(base.text || rawText || '');
    const rugs = structuredRugs(text, Array.isArray(base.rugs) ? base.rugs : []);
    return {
      ...base,
